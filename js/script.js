...
document.addEventListener("DOMContentLoaded", function () {
    ...
    link.addEventListener("click", function (e) {
      ...
      if (targetSection && typeof window.fullpageScroll !== "undefined") {
        ...
        const sectionIndex = sections.findIndex(
          (section) => section === targetSection || section.id === sectionId
        );
  ...
  const anchorLinks = document.querySelectorAll(
    'a[href^="#"]:not(.nav-link[data-section]):not(.sec6-campus-link)'
  );
  ...
  function handleSec2Animation() {
    if (sec2 && sec2Content) {
      ...
      if (
        ...
        window.scrollY < sec2Top + sec2Height
      ) {
  ...
  function handleSec5Animation() {
    if (sec5 && sec5Text) {
      ...
      if (
        window.scrollY >= triggerPoint &&
        window.scrollY < sec5Top + sec5Height
      ) {
  ...
  const flashOverlayMobile = document.querySelector(
    ".sec3-mobile-flash-overlay"
  );
  ...
  if (isMobile) {
    totalDuration =
      MOBILE_DURATIONS.textTitle +
      MOBILE_DURATIONS.textSubtitle +
      MOBILE_DURATIONS.textCompleteWait +
      MOBILE_DURATIONS.campusGridFadeIn +
      MOBILE_DURATIONS.campusGridWait +
      MOBILE_DURATIONS.cardMerge +
      MOBILE_DURATIONS.textHide +
      MOBILE_DURATIONS.whiteTransition +
      MOBILE_DURATIONS.mergeToCircle +
      MOBILE_DURATIONS.whiteFadeOut +
      MOBILE_DURATIONS.backgroundTransition;
  } else {
    // 마지막 캠퍼스 페이드인 완료 시간 계산 (춘천: 0ms, 삼석: 300ms, 강릉: 600ms, 원주: 900ms + 600ms 페이드인)
    const lastCampusFadeInEnd =
      Math.max(...CAMPUS_FADE_DELAYS) + DURATIONS.fadeIn;
    totalDuration =
      lastCampusFadeInEnd +
      DURATIONS.textDelay +
      DURATIONS.textTitle +
      DURATIONS.textSubtitle +
      DURATIONS.textCompleteWait +
      DURATIONS.cardMerge +
      DURATIONS.textHide +
      DURATIONS.whiteTransition +
      DURATIONS.mergeToCircle +
      DURATIONS.whiteFadeOut +
      DURATIONS.backgroundTransition;
  }
  ...
  function checkSec3Active() {
    ...
    if (isMobile && sec3Mobile) {
      ...
      return (
        scrollY >= sec3MobileTop - headerHeight &&
        scrollY < sec3MobileTop + sec3Mobile.offsetHeight
      );
    }
    ...
    return (
      Math.abs(sec3Center - viewportCenter) < threshold &&
  ...
  function animateSec3() {
    ...
    if (isMobile) {
      ...
      campusGridEndTime =
        campusGridStartTime + MOBILE_DURATIONS.campusGridFadeIn;
      const campusGridWaitEndTime =
        campusGridEndTime + MOBILE_DURATIONS.campusGridWait;
      ...
      textEndTime =
        textStartTime + DURATIONS.textTitle + DURATIONS.textSubtitle;
      textProgress =
        elapsed < textStartTime
          ? 0
          : elapsed >= textEndTime
    ...
    const rawMergeProgress =
      elapsed < mergeStartTime
        ? 0
        : elapsed >= mergeEndTime
        ? 1
    ...
    const cardMergeDuration = isMobile
      ? MOBILE_DURATIONS.cardMerge
      : DURATIONS.cardMerge;
    const whiteStartTime =
      mergeStartTime + cardMergeDuration * whiteStartProgress;
    const whiteEndTime = mergeEndTime; // 합쳐지기가 완료될 때까지
    // 서서히 강렬해지도록 부드러운 easing 적용
    const rawWhiteProgress =
      mergeProgress < whiteStartProgress
        ? 0
        : mergeProgress >= 1
        ? 1
    ...
    const whiteFadeOutDuration = isMobile
      ? MOBILE_DURATIONS.whiteFadeOut
      : DURATIONS.whiteFadeOut;
    const whiteFadeOutStartTime = mergeCircleEndTime;
    const whiteFadeOutEndTime = whiteFadeOutStartTime + whiteFadeOutDuration;
    const whiteFadeOutProgress =
      elapsed < whiteFadeOutStartTime
        ? 0
        : elapsed >= whiteFadeOutEndTime
        ? 1
        : (elapsed - whiteFadeOutStartTime) /
          (whiteFadeOutEndTime - whiteFadeOutStartTime);

    // 8. 배경 전환 (하얀색 페이드아웃과 동시에 시작)
    const backgroundTransitionDuration = isMobile
      ? MOBILE_DURATIONS.backgroundTransition
      : DURATIONS.backgroundTransition;
    const backgroundTransitionStartTime = whiteFadeOutStartTime;
    const backgroundTransitionEndTime =
      backgroundTransitionStartTime + backgroundTransitionDuration;
    const backgroundTransitionProgress =
      elapsed < backgroundTransitionStartTime
        ? 0
        : elapsed >= backgroundTransitionEndTime
        ? 1
        : (elapsed - backgroundTransitionStartTime) /
          (backgroundTransitionEndTime - backgroundTransitionStartTime);
    ...
    const badgeFadeOutProgress =
      elapsed < badgeFadeOutStartTime
        ? 0
        : elapsed >= badgeFadeOutEndTime
        ? 1
        : (elapsed - badgeFadeOutStartTime) /
          (badgeFadeOutEndTime - badgeFadeOutStartTime);

    // 카드 페이드 인 + 둥둥 떠오름
    campuses.forEach((campus, index) => {
      const initialTops = [131, 131, 586.1, 586.1];
      ...
      if (isMobile) {
        ...
        const textEndTime =
          MOBILE_DURATIONS.textTitle + MOBILE_DURATIONS.textSubtitle;
        const textWaitEndTime = textEndTime + MOBILE_DURATIONS.textCompleteWait;
        ...
        if (elapsed < campusFadeStartTime) {
          ...
          individualFadeProgress =
            (elapsed - campusFadeStartTime) / DURATIONS.fadeIn;
        }
        ...
        const mobileStartPositions = [
          {
            percent: chuncheonStartPercent,
            px: chuncheonStartPx,
            y: firstRowTop,
          }, // chuncheon(0): 첫 번째 줄 왼쪽
          {
            percent: chuncheonStartPercent,
            px: chuncheonStartPx + campusWidth + gridGap,
            y: firstRowTop,
          }, // gangneung(1): 첫 번째 줄 오른쪽
          {
            percent: chuncheonStartPercent,
            px: chuncheonStartPx,
            y: secondRowTop,
          }, // samcheok(2): 두 번째 줄 왼쪽
          {
            percent: chuncheonStartPercent,
            px: chuncheonStartPx + campusWidth + gridGap,
            y: secondRowTop,
        ...
        if (elapsed < mergeStartTime) {
          ...
          const currentPercent =
            startPos.percent +
            (finalPercent - startPos.percent) * adjustedProgress;
          const currentPx =
            startPos.px + (finalPx - startPos.px) * adjustedProgress;

          // Y축: 초기 위치에서 중앙으로 이동
          const initialY = startPos.y;
          const finalCenterY = contentHeight * 0.5;
          const centerY =
            initialY + (finalCenterY - initialY) * adjustedProgress;
        ...
        const adjustedProgress = Math.min(
          1,
          mergeProgress + easingOffsets[index]
        );
        ...
        const yAdjustedProgress = Math.min(
          1,
          mergeProgress + yEasingOffsets[index]
        );
      ...
      if (mergeProgress > 0) {
        ...
        if (whiteProgress > 0) {
          ...
          const glowOpacity = Math.min(
            0.98,
            overlayOpacity + additionalOpacity
          );
        ...
        if (whiteProgress > 0) {
          ...
          if (campusInfo) {
            ...
            const dotFadeProgress =
              whiteProgress < dotFadeStart
                ? 0
          ...
          if (overlayOpacity >= 0.7) {
            ...
            // 우측 원 페이드아웃
            if (isRightCampus) {
              campus.style.opacity =
                Math.min(1, fadeInProgress) * fadeOutOpacity;
              campus.style.transition = "opacity 0.3s ease-out";
            }
          ...
          if (campusName && campusName.dataset.moved === "true") {
            ...
            if (
              campusInfo &&
              (campusName.parentElement === imageWrapper ||
                campusName.parentElement === campus)
            ) {
    ...
    if (sec3Title && sec3Subtitle) {
      ...
      const subtitleProgress = Math.min(
        1,
        Math.max(0, (textProgress - 0.5) / 0.5)
      );
    ...
    if (flashOverlay) {
      if (mergeCircleProgress > 0) {
        ...
        flashOverlay.style.boxShadow = `0 0 ${shadowBlur}px rgba(255, 255, 255, ${
          0.8 + mergeCircleProgress * 0.2
        })`;
    ...
    if (sec3 && backgroundTransitionProgress > 0) {
      ...
      const bgImageOverlay = sec3.querySelector(
        ".sec3-background-image-overlay"
      );
      if (bgImageOverlay) {
        // 이미지가 페이드인되면서 나타남
    ...
    if (textOverlay) {
      ...
      if (backgroundTransitionProgress > 0) {
        ...
        const textFadeProgress =
          backgroundTransitionProgress < textFadeStart
            ? 0
            : (backgroundTransitionProgress - textFadeStart) /
              (1 - textFadeStart);
        ...
        // 텍스트 색상을 하얀색으로 변경
        const textLines = textOverlay.querySelectorAll(
          ".sec3-text-overlay-line1, .sec3-text-overlay-line2"
        );
    ...
    isAnimationFullyComplete =
      backgroundTransitionProgress >= 1 && textOverlayOpacity >= 1;
  ...
  function animateSec3Mobile() {
    ...
    if (
      !campusesContainer.dataset.initialPositionsCalculated ||
      elapsed >= mergeStartTime
    ) {
      ...
      const secondRowY =
        containerTop + campusHeight + gridGap + campusHeight / 2;
    ...
    const mobileStartPositions = [
      // 춘천: 왼쪽 위
      {
        x: parseFloat(campusesContainer.dataset.chuncheonX),
        y: parseFloat(campusesContainer.dataset.chuncheonY),
      },
      // 강릉: 오른쪽 위
      {
        x: parseFloat(campusesContainer.dataset.gangneungX),
        y: parseFloat(campusesContainer.dataset.gangneungY),
      },
      // 삼척: 왼쪽 아래
      {
        x: parseFloat(campusesContainer.dataset.samcheokX),
        y: parseFloat(campusesContainer.dataset.samcheokY),
      },
      // 원주: 오른쪽 아래
      {
        x: parseFloat(campusesContainer.dataset.wonjuX),
        y: parseFloat(campusesContainer.dataset.wonjuY),
      },
    ];
    ...
    campusesMobile.forEach((campus, index) => {
      ...
      const imageWrapper = campus.querySelector(
        ".sec3-mobile-campus-image-wrapper"
      );
    ...
    if (flashOverlayMobile) {
      if (mergeCircleProgress > 0) {
        ...
        flashOverlayMobile.style.boxShadow = `0 0 ${shadowBlur}px rgba(255, 255, 255, ${
          0.8 + mergeCircleProgress * 0.2
        })`;
    ...
    if (sec3Mobile && backgroundTransitionProgress > 0) {
      ...
      const bgImageOverlay = sec3Mobile.querySelector(
        ".sec3-mobile-background-image-overlay"
      );
      if (bgImageOverlay) {
        bgImageOverlay.style.opacity = backgroundTransitionProgress;
      }
      ...
      const bgOverlay = sec3Mobile.querySelector(
        ".sec3-mobile-background-overlay"
      );
      if (bgOverlay) {
        bgOverlay.style.opacity = 1 - backgroundTransitionProgress;
      }
    ...
    if (textOverlayMobile) {
      if (backgroundTransitionProgress > 0) {
        ...
        const textLines = textOverlayMobile.querySelectorAll(
          ".sec3-mobile-text-overlay-line1, .sec3-mobile-text-overlay-line2"
        );
        textLines.forEach((line) => {
          line.style.color = "white";
        });
    ...
    if (
      ...
      if (delta < 0) {
        ...
        if (virtualScrollProgress >= 1) {
          ...
          virtualScrollProgress = Math.max(
            0,
            Math.min(1, virtualScrollProgress)
          );
        }
  ...
  const sec1Subtitle1Wrapper = document.querySelector(
    ".sec1-subtitle-1-wrapper"
  );
  ...
  if (logoTrack) {
    ...
    function setupMarqueeAnimation() {
      ...
      if (allItems.length >= 4) {
        ...
        logoTrack.style.setProperty(
          "--sec4-marquee-distance",
          `${totalWidth}px`
        );
  ...
  if (heartIcon && supportCount) {
    ...
    let isFirstLoad = true; // [핵심] 처음 로딩인지 확인하는 변수
    
    // [기능 1] 숫자 롤링 애니메이션 (부드럽게 증가)
    function animateValue(start, end, duration) {
      ...
      const realStepTime = Math.max(stepTime, 20);

      const timer = setInterval(function () {
        current += increment;
        supportCount.textContent = current.toLocaleString();
        if (current == end) {
          clearInterval(timer);
    ...
    function fetchLatestCount() {
      fetch(API_URL)
        ...
        .then((data) => {
          ...
          if (isFirstLoad) {
            // [핵심 변경점] 첫 로딩일 때는 애니메이션 없이 바로 꽂아넣음
            currentDisplayValue = serverTotal;
            supportCount.textContent = serverTotal.toLocaleString();
            isFirstLoad = false; // 이제 첫 로딩 아니라고 표시
          } else {
            // 첫 로딩이 아닐 때만(5초 뒤 갱신 등) 애니메이션 실행
            if (serverTotal > currentDisplayValue) {
              animateValue(currentDisplayValue, serverTotal, 1000);
            }
          }
        })
        .catch((err) => console.log("데이터 확인 중..."));
    }

    // --- 실행 ---
    fetchLatestCount(); // 1. 접속하자마자 실행
    setInterval(fetchLatestCount, 5000); // 2. 5초마다 자동 갱신

    // 3. 클릭 이벤트
    heartIcon.addEventListener("click", function () {
      let sessionCount =
        Number(sessionStorage.getItem("mySessionLikeCount")) || 0;

      // 20회 제한 체크
      if (sessionCount >= MAX_LIKES) {
        alert("응원은 한 번에 최대 20번까지 가능해요");
        return;
      }

      sessionCount++;
      sessionStorage.setItem("mySessionLikeCount", sessionCount);

      // 즉시 반영 (+1)
      currentDisplayValue++;
      supportCount.textContent = currentDisplayValue.toLocaleString();
      ...
      fetch(API_URL, {
        ...
        .then((res) => res.json())
        .then((data) => {
          // 서버 숫자가 더 크면 자연스럽게 맞춤 (이때는 애니메이션 작동)
          if (data.total > currentDisplayValue) {
            animateValue(currentDisplayValue, data.total, 500);
          }
        });
    });
  }
